...
namespace mcd
{
	EmittedControlParameter::EmittedControlParameter(db::CompositeAttribute* parent, const std::string& name, ControlParameter* controlParameter)
		...
		, m_controlParameter(std::make_unique<db::Pointer<ControlParameter>>(this, name, controlParameter))
		, m_outputPin(std::make_unique<DataPin>(this, "OutputPin", controlParameter->getType()))
	{

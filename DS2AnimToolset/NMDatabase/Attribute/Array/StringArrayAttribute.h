...
namespace db
{
	class StringArrayAttribute : public ArrayAttribute
	{
		std::vector<std::string> m_values;

	public:
		StringArrayAttribute(Attribute* parent, std::string name, std::string label) :
			ArrayAttribute(parent, name, label, "stringArray") {
		};
		...
		std::string operator[] (uint32_t idx) const
		{
			if (idx < 0 || idx >= static_cast<int>(m_values.size()))
				throw std::out_of_range("StringArrayAttribute::operator[] - Index out of range");

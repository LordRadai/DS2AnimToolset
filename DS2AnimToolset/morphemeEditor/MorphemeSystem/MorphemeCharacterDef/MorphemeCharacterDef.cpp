...
void MorphemeCharacterDef::addAnimation(const char* filename, MR::RigToAnimMap* rigToAnimMap, std::vector<ME::EventTrackExport*>& exportedTracks, int animSetIdx)
{
    g_appLog->debugMessage(MsgLevel_Debug, "\tLoading animation \"%s\"\n", filename);

    int idx = m_anims.size();
    
    m_anims.push_back(AnimObject::createFromMorphemeAssets(this, this->m_netDef->getRig(animSetIdx), rigToAnimMap, filename, idx, exportedTracks));
}

    ...
    DirectX::GXMVECTOR color, bool fill)
{
    ...
    Vector3 center_mid = Vector3((center_bot + center_top) / 2);
    ...
    bot_vertex.color = Vector4(v_color);
    ...
    top_vertex.color = Vector4(v_color);
    ...
    for (int i = 0; i < numStacks; i++)
    {
        ...
        for (int j = 0; j < numSlices; j++)
        {
            ...
            top_vertices[i][j].color = v_color;
    ...
    for (int i = 0; i < numStacks; i++)
    {
        ...
        for (int j = 0; j < numSlices; j++)
        {
            ...
            bot_vertices[i][j].color = v_color;
    ...
    if (fill)
    {
        for (size_t i = 0; i < vertex_idx; i++)
            vertices[i].color = Vector4(color);

        std::vector<UINT16> indices_fill;

        for (size_t i = 0; i < numSlices; i++)
        {
            indices_fill.push_back(0);
            indices_fill.push_back(numSlices * (numStacks - 1) + i + 1);

            if (numSlices * (numStacks - 1) + i + 2 < numSlices * (numStacks - 1) + numSlices + 1)
                indices_fill.push_back(numSlices * (numStacks - 1) + i + 2);
            else
                indices_fill.push_back(numSlices * (numStacks - 1) + 2);
        }
        ...
        for (size_t i = 0; i < numStacks; i++)
        {
            for (size_t j = 0; j < numSlices; j++)
            {
                if (i + 1 < numStacks)
                {
                    indices_fill.push_back(i * numSlices + j + 1);
                    indices_fill.push_back((i + 1) * numSlices + j + 1);

                    if (i * numSlices + j + 2 < i * numSlices + numSlices + 1)
                        indices_fill.push_back(i * numSlices + j + 2);
                    else
                        indices_fill.push_back(i * numSlices + 1);

                    indices_fill.push_back((i + 1) * numSlices + j + 1);

                    if ((i + 1) * numSlices + j + 2 < (i + 1) * numSlices + numSlices + 1)
                    {
                        indices_fill.push_back(i * numSlices + j + 2);
                        indices_fill.push_back((i + 1) * numSlices + j + 2);
                    }
                    else
                    {
                        indices_fill.push_back(i * numSlices + 1);
                        indices_fill.push_back((i + 1) * numSlices + 1);
                    }
                }
            }
        }
        ...
        for (size_t i = 0; i < numSlices; i++)
        {
            ...
            indices_fill.push_back(i + 1 + numSegments + numSlices);

            if (i + 2 < numSlices + 1)
                indices_fill.push_back(i + 2 + numSegments + numSlices);
            else
                indices_fill.push_back(1 + numSegments + numSlices);
        ...
        for (size_t i = 0; i < numStacks; i++)
        {
            for (size_t j = 0; j < numSlices; j++)
            {
                if (i + 1 < numStacks)
                {
                    indices_fill.push_back(i * numSlices + j + 1 + numSegments + numSlices);
                    indices_fill.push_back((i + 1) * numSlices + j + 1 + numSegments + numSlices);

                    if (i * numSlices + j + 2 < i * numSlices + numSlices + 1)
                        indices_fill.push_back(i * numSlices + j + 2 + numSegments + numSlices);
                    else
                        indices_fill.push_back(i * numSlices + 1 + numSegments + numSlices);
                    ...
                    {
                        indices_fill.push_back(i * numSlices + j + 2 + numSegments + numSlices);
                        indices_fill.push_back((i + 1) * numSlices + j + 2 + numSegments + numSlices);
                    }
                    else
                    {
                        indices_fill.push_back(i * numSlices + 1 + numSegments + numSlices);
                        indices_fill.push_back((i + 1) * numSlices + 1 + numSegments + numSlices);
                    }
                }
            }
        }
        ...
        for (size_t i = 0; i < numSlices; i++)
        {
            indices_fill.push_back(vertex_idx - 1);
            indices_fill.push_back(numSlices * (numStacks - 1) + i + 1 + numSegments + numSlices);

            if (numSlices * (numStacks - 1) + i + 2 < numSlices * (numStacks - 1) + numSlices + 1 + numSegments + numSlices)
                indices_fill.push_back(numSlices * (numStacks - 1) + i + 2 + numSegments + numSlices);
            else
                indices_fill.push_back(numSlices * (numStacks - 1) + 2 + numSegments + numSlices);
        }
...
void XM_CALLCONV DX::DrawSphere(DirectX::PrimitiveBatch<DirectX::VertexPositionColor>* batch,
    ...
    DirectX::GXMVECTOR color, bool fill)
{
    DX::DrawCapsule(batch, world, Vector3::Zero, Vector3::Zero, radius, color, fill);
}
...
void XM_CALLCONV DX::Draw3DArc(DirectX::PrimitiveBatch<DirectX::VertexPositionColor>* batch,
    DirectX::SimpleMath::Vector3 rotation, DirectX::SimpleMath::Vector3 center, float radius, float angle, float m_height, bool is_front,
    DirectX::GXMVECTOR color)
{
    ...
    Vector3 center_top = Vector3(center.x, center.y + m_height, center.z);
    Vector3 center_bot = Vector3(center.x, center.y - m_height, center.z);
    ...
    for (size_t i = 0; i < sphere_seg; i++)
        vertices.push_back(VertexPositionColor(top_vertices_1[i], color));

    for (size_t i = 0; i < sphere_seg; i++)
        vertices.push_back(VertexPositionColor(top_vertices_2[i], color));
    ...
    for (size_t i = 0; i < sphere_seg; i++)
        vertices.push_back(VertexPositionColor(bottom_vertices_1[i], color));

    for (size_t i = 0; i < sphere_seg; i++)
        vertices.push_back(VertexPositionColor(bottom_vertices_2[i], color));

    vertices.push_back(VertexPositionColor(center_top_vertex, color));
    vertices.push_back(VertexPositionColor(center_bot_vertex, color));
...
void XM_CALLCONV DX::DrawModel(DirectX::PrimitiveBatch<DirectX::VertexPositionNormalColor>* batch,
    DirectX::XMMATRIX world, FlverModel* model)
{
    ...
    for (size_t meshIdx = 0; meshIdx < numMeshes; meshIdx++)
    {
        ...
        for (size_t i = 0; i < numVertices; i++)
        {
            if (i < numVertices)
            {
                vertices.push_back(VertexPositionNormalColor(Vector3::Transform(model->getVertex(meshIdx, i)->vertexData.position, world), Vector3::Transform(model->getVertex(meshIdx, i)->vertexData.normal, world), model->getVertex(meshIdx, i)->vertexData.color));
                indices.push_back(i);
            }
        }
...
void XM_CALLCONV DX::DrawModelWireframe(DirectX::PrimitiveBatch<DirectX::VertexPositionColor>* batch,
    DirectX::XMMATRIX world, FlverModel* model, Vector4 color)
{
    ...
    for (size_t meshIdx = 0; meshIdx < numMeshes; meshIdx++)
    {
        ...
        vertices.reserve(numMeshes);
        indices.reserve(numMeshes * 2);

        for (int i = 0; i < numVertices; i += 3)
        {
            if (i + 1 < numVertices && i + 2 < numVertices)
            {
                VertexPositionColor v1 = VertexPositionColor(Vector3::Transform(model->getVertex(meshIdx, i)->vertexData.position, world), color);
                VertexPositionColor v2 = VertexPositionColor(Vector3::Transform(model->getVertex(meshIdx, i + 1)->vertexData.position, world), color);
                VertexPositionColor v3 = VertexPositionColor(Vector3::Transform(model->getVertex(meshIdx, i + 2)->vertexData.position, world), color);
        ...
        try
        {
            batch->DrawIndexed(D3D_PRIMITIVE_TOPOLOGY_LINELIST, indices.data(), indices.size(), vertices.data(), vertices.size());
        }
        catch (const std::exception& e)
        {
            g_appLog->panicMessage(e.what());

...
#else
  for (uint32_t i = 0; i < m_numElements; ++i) m_elements[i] = (int32_t)fabs((float)m_elements[i]);
#endif
}
...
#else
    if ((int32_t)fabs((float)m_elements[i]) < v) m_elements[i] = 0;
#endif
  }
}
...
bool Vector<ScalarType>::isZero(ScalarType threshold) const
{
  for (uint32_t i = 0 ; i < m_numElements ; ++i)
  {
    if (fabs(m_elements[i]) > threshold) return false;
  }
  return true;
}

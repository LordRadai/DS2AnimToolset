...
void TransformsAccumulator::maxAbsQuatElements(NMP::Quat& quat) const
{
  ...
  for (uint32_t i = 0; i < m_numChannelSets; ++i)
  {
    quat.x = NMP::maximum(quat.x, (float)fabs(m_channelQuat[i].x));
    quat.y = NMP::maximum(quat.y, (float)fabs(m_channelQuat[i].y));
    quat.z = NMP::maximum(quat.z, (float)fabs(m_channelQuat[i].z));
    quat.w = NMP::maximum(quat.w, (float)fabs(m_channelQuat[i].w));
  }
}
...
void TransformsAccumulator::maxAbsPosElements(NMP::Vector3& pos) const
{
  ...
  for (uint32_t i = 0; i < m_numChannelSets; ++i)
  {
    pos.x = NMP::maximum(pos.x, (float)fabs(m_channelPos[i].x));
    pos.y = NMP::maximum(pos.y, (float)fabs(m_channelPos[i].y));
    pos.z = NMP::maximum(pos.z, (float)fabs(m_channelPos[i].z));
  }
}

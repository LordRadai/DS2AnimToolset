  ...
  const NMP::Memory::Format& assetMemReqs)
{
  ...
  NMP_ASSERT((assetMemReqs.alignment & 0xFFFFFFFF) <= NMP_VECTOR_ALIGNMENT);

  // Pad to the asset's alignment requirements.
  if (!padStreamToAlignment(assetMemReqs.alignment & 0xFFFFFFFF))
  {
    return false;
  }
  ...
  NMP::Memory::Format&    assetMemReqs)
{
  ...
  if ((header->m_assetMemReqs.alignment & 0xFFFFFFFF) > bufferResource.format.alignment)
  {
    NM_LOG_MESSAGE(
      ...
      bufferResource.format.alignment & 0xFFFFFFFF);
    return false;
  }
